...
def get_normalized_pixel_grid(opt):
    y_range = (
        (torch.arange(opt.H, dtype=torch.float32, device=opt.device) + 0.5) / opt.H * 2
        - 1
    ) * (opt.H / max(opt.H, opt.W))
    x_range = (
        (torch.arange(opt.W, dtype=torch.float32, device=opt.device) + 0.5) / opt.W * 2
        - 1
...
def warp_corners(opt, warp_param):
    ...
    corners = torch.tensor(corners, dtype=torch.float32, device=opt.device).repeat(
        opt.batch_size, 1, 1
    )
...
class Lie:
    ...
    def se2_to_SE2(self, delta):  # [...,3]
        ...
        V = torch.stack([torch.cat([A, -B], dim=-1), torch.cat([B, A], dim=-1)], dim=-2)
    ...
    def SE2_to_se2(self, Rt, eps=1e-7):  # [...,2,3]
        ...
        invV = (
            torch.stack([torch.cat([A, B], dim=-1), torch.cat([-B, A], dim=-1)], dim=-2)
            / denom
        )
        u = (invV @ t)[..., 0]
