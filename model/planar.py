...
class Model(base.Model):
    ...
    def build_networks(self, opt):
        ...
        self.graph.warp_param = torch.nn.Embedding(opt.batch_size, opt.warp.dof).to(
            opt.device
        )
    ...
    def visualize_patches(self, opt, warp_param):
        ...
        for i, corners in enumerate(corners_all):
            ...
            draw.line(
                [P[0], P[1], P[2], P[3], P[0]], fill=tuple(self.box_colors[i]), width=3
            )
    ...
    def visualize(self, opt, var, step=0, split="train"):
        ...
        frame_cat = (
            (torch.cat([frame, frame2], dim=1) * 255).byte().permute(1, 2, 0).numpy()
        )
