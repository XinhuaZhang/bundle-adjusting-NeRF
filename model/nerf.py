...
class Model(base.Model):
    ...
    def generate_videos_synthesis(self, opt, eps=1e-10):
        ...
        if opt.data.dataset == "blender":
            ...
            idx_center = (
                (poses - poses.mean(dim=0, keepdim=True))[..., 3].norm(dim=-1).argmin()
            )
...
class Graph(base.Graph):
    ...
    def sample_depth(self, opt, batch_size, num_rays=None):
        ...
        rand_samples = (
            torch.rand(
                batch_size, num_rays, opt.nerf.sample_intvs, 1, device=opt.device
            )
        ...
        depth_samples = (
            rand_samples / opt.nerf.sample_intvs * (depth_max - depth_min) + depth_min
        )  # [B,HW,N,1]
        depth_samples = dict(
