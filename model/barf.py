...
class Model(nerf.Model):
    ...
    def setup_optimizer(self, opt):
        ...
        self.optim_pose = optimizer(
            [dict(params=self.graph.se3_refine.parameters(), lr=opt.optim.lr_pose)]
        )
        # set up scheduler
    ...
    def train_iteration(self, opt, var, loader):
        ...
        if opt.optim.warmup_pose:
            ...
            self.optim_pose.param_groups[0]["lr_orig"] = self.optim_pose.param_groups[
                0
            ][
                "lr"
        ...
        if opt.optim.warmup_pose:
            self.optim_pose.param_groups[0]["lr"] = self.optim_pose.param_groups[0][
                "lr_orig"
            ]  # reset learning rate
        if opt.optim.sched_pose:
    ...
    def get_all_training_poses(self, opt):
        ...
        if opt.data.dataset == "blender" or opt.data.dataset == "llff":
            pose = pose_GT
            if opt.camera.noise:
    ...
    def evaluate_test_time_photometric_optim(self, opt, var):
        ...
        iterator = tqdm.trange(
            opt.optim.test_iter, desc="test-time optim.", leave=False, position=1
        )
        for it in iterator:
        ...
        for ep in range(0, opt.max_iter + 1, opt.freq.ckpt):
            ...
            if opt.data.dataset in ["blender", "llff"]:
                ...
                dict(
                    ...
                    opt, fig, pose_aligned, pose_ref=pose_ref, path=cam_path, ep=ep
                )
            else:
